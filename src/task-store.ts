...
import type { CompletedTaskRecord, Task, TaskProgress, TaskStatus, TaskStoreData } from "./types.js";
...
export class TaskStore {
  ...
  private completedTaskHistory = new Map<string, CompletedTaskRecord>();
  ...
  private load(): void {
    ...
    try {
      ...
      this.historyInitialized = data.historyInitialized === true;
      this.completedTaskHistory.clear();
      for (const record of data.completedTaskHistory ?? []) {
        if (typeof record?.id === "string" && HIERARCHICAL_TASK_ID.test(record.id)) {
          this.completedTaskHistory.set(record.id, {
            id: record.id,
  ...
  private reconcileSubtaskState(): void {
    for (const task of [...this.tasks.values(), ...this.completedTaskHistory.values()]) {
      const rootOrdinal = Number(task.id.split(".")[0]);
      if (Number.isSafeInteger(rootOrdinal)) this.nextId = Math.max(this.nextId, rootOrdinal + 1);
  ...
  private save(): void {
    ...
    const data: TaskStoreData = {
      ...
      completedTaskHistory: Array.from(this.completedTaskHistory.values()),
      historyInitialized: this.historyInitialized,
  ...
  /** Summary input is independent of retained/displayed rows. Live records always win. */
  getProgress(): TaskProgress[] {
    if (this.filePath) this.load();
    const completed: TaskProgress[] = Array.from(this.completedTaskHistory.values())
      .filter(task => !this.tasks.has(task.id))
      .map(task => ({ ...task, status: "completed", blockedBy: [] }));
    return [...completed, ...this.tasks.values()];
  }

  /** Import confirmed legacy completions once, without resurrecting rows or guessing from ID gaps. */
  initializeCompletedHistory(recover: () => readonly CompletedTaskRecord[]): void {
    if (this.historyInitialized) return;
    this.withLock(() => {
      if (this.historyInitialized) return;
      for (const task of recover()) {
        if (!this.tasks.has(task.id)) this.completedTaskHistory.set(task.id, { ...task });
      }
      this.reconcileSubtaskState();
  ...
  /** Automatic cleanup removes rows but keeps their contribution to progress totals. */
  archiveCompleted(taskIds?: readonly string[]): number {
    return this.withLock(() => {
      const selected = taskIds ? new Set(taskIds) : undefined;
      const removed = new Set<string>();
      for (const [id, task] of this.tasks) {
        if (task.status !== "completed" || (selected && !selected.has(id))) continue;
        this.completedTaskHistory.set(id, { id, parentTaskId: task.parentTaskId });
        this.tasks.delete(id);
        removed.add(id);
      }
      for (const task of this.tasks.values()) {
        task.blocks = task.blocks.filter(id => !removed.has(id));
        task.blockedBy = task.blockedBy.filter(id => !removed.has(id));
      }
      return removed.size;
    });
  }
  ...
  update(id: string, fields: {
    ...
    return this.withLock(() => {
      ...
      if (!task) {
        const deleted = fields.status === "deleted" && this.completedTaskHistory.delete(id);
        return { task: undefined, changedFields: deleted ? ["deleted"] : [], warnings: [] };
      }
      ...
      if (fields.status === "deleted") {
        this.tasks.delete(id);
        this.completedTaskHistory.delete(id);
  ...
  delete(id: string): boolean {
    return this.withLock(() => {
      const archived = this.completedTaskHistory.delete(id);
      if (!this.tasks.delete(id)) return archived;
  ...
  clearAll(): number {
    return this.withLock(() => {
      const count = new Set([...this.tasks.keys(), ...this.completedTaskHistory.keys()]).size;
      this.tasks.clear();
      this.completedTaskHistory.clear();
      this.historyInitialized = true;
      return count;
  ...
  /** Keep initialized empty stores as a reset marker so reload cannot revive cleared history. */
  deleteFileIfEmpty(): boolean {
    if (!this.filePath || this.tasks.size > 0 || this.completedTaskHistory.size > 0 || this.historyInitialized) return false;
  ...
  clearCompleted(): number {
    return this.withLock(() => {
      let count = this.completedTaskHistory.size;
      this.completedTaskHistory.clear();
      this.historyInitialized = true;
      for (const [id, task] of this.tasks) {

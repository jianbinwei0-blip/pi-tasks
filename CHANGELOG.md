...
### Fixed
- **Completed counts survive cleanup and reload** — automatic cleanup now archives minimal completion records instead of erasing progress. The widget includes those records in total/done counts even when no task rows remain. Legacy default session stores recover confirmed completions once from successful task status updates, respecting later reopening and task-tool deletion without guessing from ID gaps. Explicit clears reset history and persist a marker so reload cannot revive it; current Pi session-start events restore the widget immediately.

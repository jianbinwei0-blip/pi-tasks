...
Widget stats use 24-hour clock times and compact stopwatch durations: completed tasks show `start → end Δduration`, while running tasks show `start Δelapsed`. The token group keeps `↑` input, `↓` output, `Σ` total, and `⨀` task-wide cache hit ratio together; `t/s` is average output-token throughput.
